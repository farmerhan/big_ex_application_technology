...
<?php
    ...
    $get_orders = "select * from don_hang";
    ...
    while ($row_orders = mysqli_fetch_array($run_orders)) {
        ...
        $c_id = $row_orders['ma_kh'];

        $product_id = $row_orders['ma_sp'];

        $qty = $row_orders['so_luong'];

        $size = $row_orders['kich_co'];

        $order_status = $row_orders['trang_thai_don_hang'];

        $get_products = "select * from do_the_thao where ma_do_the_thao='$product_id'";

        $run_products = mysqli_query($con, $get_products);

        $row_products = mysqli_fetch_array($run_products);

        $product_title = $row_products['ten_sp'];
